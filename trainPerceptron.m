function [w, fAvg] = trainPerceptron(examples, decodeFunc, C, options, w)
%
% Trains an MRF using structured Perceptron.
%
...
if length(examples) == 1
	objFun = @(x) perceptronObj(x, examples, C, decodeFunc);
	[w,fAvg] = pgd(objFun, [], w, options);
else
	objFun = @(x, ex, t) perceptronObj(x, {ex}, C, decodeFunc);
