...
% get edge structure
edgeStruct = UGM_makeEdgeStruct(adj,[repmat(nStateY,nNode,1); repmat(nStateX,nNode,1)]);
...
for e=1:nEdge
    ...
    if (ends(1) > nNode) || (ends(2) > nNode)
        edgePot(1:nStateY,1:nStateX,e) = exp(wloc + 0.05*randn(nStateY,nStateX));
    else
        edgePot(1:nStateY,1:nStateY,e) = exp(wrel + 0.05*randn(nStateY,nStateY));
    end
end
