function [w, kappa, f] = trainVCTSM_2kappa(examples, inferFunc, C1, C2, options, w, kappa)
...
objFun = @(x, varargin) vctsmObj_log_2kappa(x, examples, C1, C2, inferFunc, varargin{:});
x0 = [w ; log(kappa)];
[x,f] = minFunc(objFun, x0, options);
w = x(1:end-2);
kappa = exp(x(end-1:end));
